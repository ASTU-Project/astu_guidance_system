    ...
    <div class="space-y-5">
        ...
        <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
            <div class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <form action="{{ route('admin.profile.update') }}" method="POST" class="mt-5 space-y-4">
                    ...
                    <div>
                        ...
                        <input id="profile-name" name="name" type="text" value="{{ old('name', $user->name) }}" class="w-full rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-700 focus:border-slate-400 focus:outline-none" required>
                    </div>

                    <div>
                        <label for="profile-email" class="mb-1 block text-sm font-medium text-slate-700">Email Address</label>
                        <input id="profile-email" name="email" type="email" value="{{ old('email', $user->email) }}" class="w-full rounded-md border border-slate-200 px-3 py-2 text-sm text-slate-700 focus:border-slate-400 focus:outline-none" required>
                    </div>
            ...
            <div class="rounded-md border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <form action="{{ route('admin.profile.password.update') }}" method="POST" class="mt-5 space-y-4">
                    @csrf
                    @method('PUT')
